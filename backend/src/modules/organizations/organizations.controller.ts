...
const brandImageUploadInterceptor = FileInterceptor('file', {
  storage: diskStorage({
    ...
    filename: (_, file, cb) => {
      const extension = file.originalname.match(/\.(png|jpe?g|webp|svg)$/i)?.[0]?.toLowerCase() || '.png';
      cb(null, `${Date.now()}-${Math.random().toString(36).slice(2)}${extension}`);
    }
  }),
  limits: { fileSize: 2 * 1024 * 1024 },
  fileFilter: (_, file, cb) => {
    if (/\.(png|jpe?g|webp|svg)$/i.test(file.originalname) && /^image\/(png|jpe?g|webp|svg\+xml)$/i.test(file.mimetype)) return cb(null, true);
    return cb(new BadRequestException('Formato invÃ¡lido. Envie PNG, JPG, WEBP ou SVG atÃ© 2MB.'), false);
  }
});
...
export class OrganizationsController {
  ...
  @Post() create(@Body() dto: CreateOrganizationDto, @CurrentUser() user: any) { return this.service.create(dto, user); }
  ...
  @Post(':id/brand-image') @UseInterceptors(brandImageUploadInterceptor)
  ...
  @Put(':id') update(@Param('id') id: string, @Body() dto: UpdateOrganizationDto, @CurrentUser() user: any, @Headers('x-organization-id') currentOrg?: string) { return this.service.update(id, dto, user, currentOrg); }
